...
func (instance *firecrackerInstance) finish(exit Exit) bool {
	finished := false
...
func (instance *firecrackerInstance) result() Exit {
	instance.mu.RLock()
	defer instance.mu.RUnlock()
...
type FirecrackerHandler struct {
	binary       string
	sandboxRoot  string
	storageRoot  string
	runtimeRoot  string
...
func (handler *FirecrackerHandler) ValidateStartRequest(
...
func NewFirecrackerHandler(
	cfg config.Config,
	binary string,
	loader OciLoader,
) (*FirecrackerHandler, error) {
...
func (handler *FirecrackerHandler) Start(
	ctx context.Context,
	...
	bundlePath, spec, err := handler.ociLoader.GenerateOci(OciLoadOptions{
	...
	if err := writeSandboxRuntimeMarker(bundlePath, config.RuntimeNameFirecracker); err != nil {
...
func (handler *FirecrackerHandler) machineSize(
	resources interface {
		GetCpuQuota() int64
		GetCpuPeriod() uint64
		GetCpuShares() uint64
...
func (handler *FirecrackerHandler) Delete(ctx context.Context, sandboxID string) error {
...
func (handler *FirecrackerHandler) runtimeDirectory(sandboxID string) string {
...
func (handler *FirecrackerHandler) cleanupRuntimeDirectory(
	sandboxID,
	apiPath string,
) error {
...
func (handler *FirecrackerHandler) List(context.Context) ([]*State, error) {
	handler.mu.RLock()
...
func (handler *FirecrackerHandler) Wait(
	ctx context.Context,
	sandboxID string,
) (Exit, error) {
	instance, err := handler.lookupInstance(sandboxID)
...
func (handler *FirecrackerHandler) lookupInstance(
	sandboxID string,
...
func (handler *FirecrackerHandler) waitCommand(
	instance *firecrackerInstance,
	command *exec.Cmd,
) {
	err := command.Wait()
	...
	exit := Exit{ExitedAt: time.Now(), ExitCode: firecrackerHostExitCode(err)}
...
func (handler *FirecrackerHandler) waitGuest(
	instance *firecrackerInstance,
) {
	state := instance.snapshot()
...
func (handler *FirecrackerHandler) stopInstance(
	instance *firecrackerInstance,
	force bool,
) {
	state := instance.snapshot()
	if !firecrackerProcessMatches(state.PID, handler.binary, state.APIPath, state.ID) {
		instance.finish(Exit{ExitedAt: time.Now(), ExitCode: state.ExitCode})
		return
	}
...
func (handler *FirecrackerHandler) persistInstance(
	instance *firecrackerInstance,
) error {
	state := instance.snapshot()
...
func (handler *FirecrackerHandler) validatePersistedState(
	sandboxID,
	bundlePath string,
	state firecrackerPersistedState,
) error {
...
func (handler *FirecrackerHandler) recoverInstances() {
...
func (handler *FirecrackerHandler) recoverState(
	state firecrackerPersistedState,
) *firecrackerInstance {
	...
	if state.Exited {
		...
		instance.finish(Exit{ExitedAt: exitTime, ExitCode: state.ExitCode})
		if firecrackerProcessMatches(state.PID, handler.binary, state.APIPath, state.ID) {
	...
	if !firecrackerProcessMatches(state.PID, handler.binary, state.APIPath, state.ID) {
		...
		instance.finish(Exit{ExitedAt: exitTime, ExitCode: 255})
		return instance
	}
	if !state.Configured {
		logrus.Warnf(
		...
		_ = signalFirecrackerProcess(state, handler.binary, syscall.SIGKILL)
		instance.finish(Exit{ExitedAt: time.Now(), ExitCode: 255})
		if err := handler.persistInstance(instance); err != nil {
...
func (handler *FirecrackerHandler) monitorRecovered(
	instance *firecrackerInstance,
) {
	...
	for range ticker.C {
		...
		if instance.finish(Exit{ExitedAt: time.Now(), ExitCode: 255}) &&
			instance.shouldPersist() {
			if err := handler.persistInstance(instance); err != nil {
